...
def postprocess():
    ...
    # Node versions 15 and 19 fail to install dependencies, if they were chosen based
    # on the node_releases.json with a 12 months offset, we set them to the next major
    # version, which is 16 and 20 respectively. Setting node_version_source to "postprocessing fix".
    commits["node_version"] = fix_node_version(commits, 15, 16)
    commits["node_version"] = fix_node_version(commits, 19, 20)
