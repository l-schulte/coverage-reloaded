...
#   - Unit tests (Jest)            → primary coverage source ✅
#   - Cypress component tests (mid-era) → not collected ❌
...
# ── 3. No test infrastructure found ──────────────────────────────────────
# Neither test:coverage nor jest are available for this commit.
# Fail loudly to avoid recording incomplete/bad coverage data.
if [ "$HAS_TEST_COVERAGE_SCRIPT" != "true" ] && [ "$IS_JEST" != "true" ]; then
    echo "ERROR: No test infrastructure detected (no test:coverage script and no jest)"
