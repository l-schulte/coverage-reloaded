        ...
        def get_container_from_package(package_json):
            if "engines" in package_json and "node" in package_json["engines"]:
                ...
                if version_match:
                    ...
                    if major_version < 12:
                        return f"node:{node_version}-buster", package_json["engines"]["node"]
                    else:
                        return f"node:{node_version}-bullseye", package_json["engines"]["node"]
            return latest_container, None
        ...
        if len(lines) == 0:
            ...
            try:
                ...
                if package_json_content:
                    ...
                    latest_container, source = get_container_from_package(package_json)
            except Exception:
                pass
