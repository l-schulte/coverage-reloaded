...
if [ "$2" = "shell" ]; then
    ...
    $EXECUTOR run --rm -it --network mining-net --cap-add=NET_ADMIN --env-file .env -v "$(pwd)/projects/$1/output:$CONTAINER_DIR/coverage" -e "revision=$3" -e "timestamp=$4" -e "package_manager=$5" -e "project_id=$7" --pids-limit 10000 $CONTAINER_NAME bash
elif [ "$2" = "debug" ]; then
    # Run an interactive container for debugging, executes bash and mounts the debug folder
    mkdir -p projects/$1/debug
    $EXECUTOR run --rm -it --network mining-net --cap-add=NET_ADMIN --env-file .env -v "$(pwd)/projects/$1/output:$CONTAINER_DIR/coverage" -v "$(pwd)/projects/$1/debug:$CONTAINER_DIR" -e "revision=$3" -e "timestamp=$4" -e "package_manager=$5" -e "project_id=$7" --pids-limit 10000 $CONTAINER_NAME bash
elif [ "$2" = "exec" ]; then
    # Run the full process non-interactively
    $EXECUTOR run --rm --network mining-net --cap-add=NET_ADMIN --env-file .env -v "$(pwd)/projects/$1/output:$CONTAINER_DIR/coverage" -e "revision=$3" -e "timestamp=$4" -e "package_manager=$5" -e "project_id=$7" --pids-limit 10000 $CONTAINER_NAME bash execute.sh
fi